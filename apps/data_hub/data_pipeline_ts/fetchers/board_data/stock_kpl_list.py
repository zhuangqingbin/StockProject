...
class KPLListFetch(BaseFetcher):
    ...
    def _read_once(self, *, trade_date: str, tag: str) -> pd.DataFrame:
        frame = self.client.call("kpl_list",
    ...
    def read_data(self, **kwargs: Any) -> pd.DataFrame:
        ...
        if not trade_date:
            raise ValueError("kpl_list fetches require trade_date")

        frames: list[pd.DataFrame] = []
        for tag in KPL_LIST_TAGS:
            frame = self._read_once(trade_date=trade_date, tag=tag)
            if frame.empty:
                continue
